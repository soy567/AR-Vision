...
class LaneNetDataProducer(object):
    ...
    def __init__(self):
        ...
        """
        self._dataset_dir = CFG.DATASET.DATA_DIR # 작업할 데이터셋 경로 가져오기 
        self._tfrecords_save_dir = ops.join(self._dataset_dir, 'tfrecords') # tfrecord 파일 저장할 경로 설정
        self._train_example_index_file_path = CFG.DATASET.TRAIN_FILE_LIST # 훈련 파일 리스트 텍스트 파일
        self._test_example_index_file_path = CFG.DATASET.TEST_FILE_LIST # 테스트 파일 리스트 텍스트 파일
        self._val_example_index_file_path = CFG.DATASET.VAL_FILE_LIST # 검증용 파일 리스트 텍스트 파일

        self._gt_image_dir = ops.join(self._dataset_dir, 'gt_image') # 데이터셋의 원본이미지 저장 경로 
        self._gt_binary_image_dir = ops.join(self._dataset_dir, 'gt_binary_image') # 데이터셋의 이진 분할 이미지 경로
        self._gt_instance_image_dir = ops.join(self._dataset_dir, 'gt_instance_image') # 데이터셋의 객체 분할 이미지 경로

        if not self._is_source_data_complete(): # 데이터셋 구성이 완벽하지 않을 경우 에러메시지 출력
            raise ValueError('Source image data is not complete, '
                             'please check if one of the image folder does not exist')

        if not self._is_training_sample_index_file_complete(): # 인덱스파일 모두 있지 않을 경우
            self._generate_training_example_index_file() # 인덱스 생성 함수 실행
    ...
    def _is_training_sample_index_file_complete(self): # 이미지 인덱스 파일(train, test, val 텍스트 파일) 모두 있는지 확인
        """
