...
class LaneNetBackEnd(cnn_basenet.CNNBaseModel):
    ...
    def inference(self, binary_seg_logits, instance_seg_logits, name, reuse):
        ...
        with tf.variable_scope(name_or_scope=name, reuse=reuse):
            ...
            with tf.variable_scope(name_or_scope='binary_seg'): # 이진 분류 처리
                binary_seg_score = tf.nn.softmax(logits=binary_seg_logits) # softmax 사용하여 예측한 결과값들 정규화 
                binary_seg_prediction = tf.argmax(binary_seg_score, axis=-1) # argmax 사용하여 입력값중 가장 큰 값 차선으로 예측

            with tf.variable_scope(name_or_scope='instance_seg'):
                pix_bn = self.layerbn( # 레이어에 BN 수행하고 저장
                    inputdata=instance_seg_logits, is_training=self._is_training, name='pix_bn')
                pix_relu = self.relu(inputdata=pix_bn, name='pix_relu') # 활성화 함수인 ReLU적용
                instance_seg_prediction = self.conv2d( # convolution 하여 최종 결과 저장
                    inputdata=pix_relu,
                    out_channel=self._embedding_dims,
                    kernel_size=1,
                    use_bias=False,
        ...
        return binary_seg_prediction, instance_seg_prediction # 결과값 리턴
