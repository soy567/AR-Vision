...
class _LaneNetCluster(object):
    ...
    def _embedding_feats_dbscan_cluster(self, embedding_image_feats):
        ...
        db = DBSCAN(eps=self._cfg.POSTPROCESS.DBSCAN_EPS, min_samples=300)
        try:
       ...
       # 이미지의 객체 레이블링
        ...
        for lane_index, coords in enumerate(lane_coords): # 인식된 lane 수 만큼 반복(coords -> 좌표)
          ...
          # [0] -> 영상 세로 크기, [1] -> 영상 가로 크기
            ...
            if(lane_index == 0): # 첫 차선 감지 시
                try:
                    horizon_idx = np.argmax(nonzero_x) # 감지된 차선 중 세로길이 가지는 픽셀 인덱스 저장
                    start_lane_pts.append([nonzero_y[horizon_idx], nonzero_x[horizon_idx]]) # 해당 좌표 리스트에 저장
                except ValueError:
                    print("차선 감지되지 않음")
                    continue

            # np.polyfit(, , 식의 차수) -> 주어진 데이터에 대해 선형회귀(데이터들 나타내는 선)식 계수 출력 (노이즈 제거하고 하나의 선으로 바꿈)
            try:
                fit_param = np.polyfit(nonzero_y, nonzero_x, 2) # 2차식으로 구한 계수
            except TypeError:
                print("차선에 해당하는 2차식 생성되지 않음!")
                continue
            fit_params.append(fit_param) # 데이터 선형회귀 값 fit_params 리스트에 추가

            [ipm_image_height, ipm_image_width] = tmp_ipm_mask.shape # 640X640
            # np.linespace(start, stop, 사이의 노드 수) -> 해당 수열 가지는 1차원 배열 생성(수평 축 좌표)
            plot_y = np.linspace(10, ipm_image_height, ipm_image_height - 10) # 총 630개 픽셀
            fit_x = fit_param[0] * plot_y ** 2 + fit_param[1] * plot_y + fit_param[2] # 차선 나타내는 2차 함수 생성
            # fit_x = fit_param[0] * plot_y ** 3 + fit_param[1] * plot_y ** 2 + fit_param[2] * plot_y + fit_param[3] # 3차 함수 사용

            lane_pts = []
            for index in range(0, plot_y.shape[0], 5): # plot_y.shape[0] -> 630
                ...
                if src_x <= 0:
                    continue
                src_y = self._remap_to_ipm_y[ # 차선 해당하는 y 값 저장
                    int(plot_y[index]), int(np.clip(fit_x[index], 0, ipm_image_width - 1))]
                src_y = src_y if src_y > 0 else 0 # 파이썬 조건부 표현식(삼항 연산자 ? : 와 비슷)

                lane_pts.append([src_x, src_y]) # lane_pis 배열에 해당 x, y 좌표추가
            src_lane_pts.append(lane_pts) # src_lane_pts 배열에 IPM 변환 사용하여 구한 차선의 전체 x, y 좌표 저장

        # 입력값에 따라 start_y 조정(mod)
        if start_lane_pts:
            start_y = self._remap_to_ipm_y[start_lane_pts[0][0], start_lane_pts[0][1]] # remap ipm 사용시 사용가능
        else:
            start_y = 240
        
        # tusimple test data sample point along y axis every 10 pixels
        source_image_width = source_image.shape[1] # 원본 이미지의 열 -> 이미의 가로 길이
        for index, single_lane_pts in enumerate(src_lane_pts): # 차선의 포인트인 src_lane_pts 순회
            ...
            if data_source == 'tusimple': # 차선 표시할 범위(세로) 지정
                start_plot_y = 230
                # start_plot_y = start_y
                end_plot_y = 720
            else:
                raise ValueError('Wrong data source now only support tusimple')

            for i in np.where(single_lane_pt_y >= start_plot_y): # 시작 픽셀 이상의 좌표만 수집
