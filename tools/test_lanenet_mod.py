...
def minmax_scale(input_arr):
    ...
    max_val = np.max(input_arr) # 배열에서 최대값 가져옴
...
def test_lanenet(image_path, weights_path):
    ...
    with sess.as_default():
        ...
        # binary_seg_image->(1, 256, 512) 3차원 맨 앞 1은 채널
...
if __name__ == '__main__':
    ...
    # placeholder 자료형의 변수 생성(텐서를 placeholder에 맵핑), 딥러닝 추론위한 텐서 생성
    input_tensor = tf.placeholder(dtype=tf.float32, shape=[1, 256, 512, 3], name='input_tensor')

    net = lanenet.LaneNet(phase='test', cfg=CFG) # 딥러닝 네트워크 그래프 설정
    # 텐서플로우 그래프 빌드, 세션 실행시 실제로 실행내용 정의
    ...
    while True:
        ...
        prevTime = time.time()
        # tf.reset_default_graph() # 이전에 get_variable 로 생성된 변수 삭제
